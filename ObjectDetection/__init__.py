...
def jersey_number(npimg):
    ...
    ball_con = {'bb': None, 'con': None, 'class': None, 'cropped': None, "text": None}
    if len(results) > 0:
        confidences = [c[4] for c in results]
        index = np.argmax(confidences)
        x, y, x1, y1 = [round(i) for i in results[index][:4]]
        ball_con['bb'] = [x, y, x1-x, y1-y]
        x,y,w,h = ball_con['bb']
        ball_con['class'] = 3
        ball_con['con'] = confidences[np.argmax(confidences)]
        ball_con['cropped']=(original[y:y+h, x:x+w])
        ball_con['text'] =ocr_it(ball_con['cropped'])
        return ball_con
    else:
        return None
